...
export const ChartRenderer: React.FC<ChartRendererProps> = ({
  ...
  const renderChart = () => {
    ...
    // ── TABLE chart type ──────────────────────────────────────────────────────
    // Renders a full-featured, scrollable HTML data table showing all columns.
    // This is the display mode for chartType === 'table' generated by AI Dashboard Builder.
    if (chartType === 'table') {
      if (!dataset.data || dataset.data.length === 0) return <EmptyChart msg="No data for Table." />;

      const allKeys = Object.keys(dataset.data[0] || {});
      const dataLimit_n = parseInt(dataLimit, 10);
      const rows = dataLimit_n > 0 ? dataset.data.slice(0, dataLimit_n) : dataset.data;
      ...
      return (
        <div className="w-full h-full overflow-auto text-sm">
            ...
            <thead className="sticky top-0 z-10 bg-muted">
              <tr>
                {allKeys.map(k => (
                  <th
                    key={k}
                    className="p-2 border border-border text-left font-semibold whitespace-nowrap text-muted-foreground uppercase tracking-wider text-xs"
                  >
                    {k.replace(/_/g, ' ')}
                  </th>
                ))}
              ...
              {rows.map((row: any, i: number) => (
                <tr key={i} className="hover:bg-muted/50 transition-colors">
                  {allKeys.map(k => (
                    <td
                      key={k}
                      className={`p-2 border border-border whitespace-nowrap ${typeof row[k] === 'number' ? 'text-right font-mono text-foreground' : 'text-muted-foreground'}`}
                    >
          ...
          {dataLimit_n > 0 && dataset.data.length > dataLimit_n && (
            <div className="p-2 text-center text-xs text-muted-foreground border-t border-border">
              Showing {dataLimit_n} of {dataset.data.length} rows
            </div>
          )}
        </div>
      );
    ...
    // ── PIVOT chart type ──────────────────────────────────────────────────────
    // 'pivot' is an alias for 'pivot_table'. The AI dashboard builder may save
    // the chart with type='pivot' while the full pivot logic lives in 'pivot_table'.
    // We normalise the type so the block below handles both seamlessly.
    const effectiveChartType = chartType === 'pivot' ? 'pivot_table' : chartType;

    if (effectiveChartType === 'pivot_table') {
